...
gfxfont_t *FontInfo::getGfxFont() {
    if (!this->gfxfont) {
        ...
        if (this->space_char >= 0) {
            ...
            msg("<debug> Appending space char to font %s, position %d, width %f", this->gfxfont->id, this->space_char,
                this->gfxfont->glyphs[this->space_char].advance);
        }
        gfxfont_fix_unicode(this->gfxfont, config_unique_unicode);

        /* optionally append a marker glyph */
        if (config_marker_glyph) {
            msg("<debug> Appending marker char to font %s, position %d, unicode %d", this->gfxfont->id,
                this->gfxfont->num_glyphs, config_marker_glyph);
...
void InfoOutputDev::drawChar(GfxState *state, double x, double y,
    ...
    if (uLen && ((u[0] >= 32 && u[0] < g->unicode) || !g->unicode)) {
        if (uLen == 2 && u[0] == 102 && u[1] == 102) {
            msg("<warning> <scireum> Reducing a multi-byte ligature to ff", 0);
            g->unicode = 0xFB00;
        } else if (uLen == 2 && u[0] == 102 && u[1] == 105) {
            msg("<warning> <scireum> Reducing a multi-byte ligature to fi", 0);
            g->unicode = 0xFB01;
        } else if (uLen == 2 && u[0] == 102 && u[1] == 108) {
            msg("<warning> <scireum> Reducing a multi-byte ligature to fl", 0);
            g->unicode = 0xFB02;
        } else if (uLen == 3 && u[0] == 102 && u[1] == 102 && u[2] == 105) {
            msg("<warning> <scireum> Reducing a multi-byte ligature to ffi", 0);
            g->unicode = 0xFB03;
        } else if (uLen == 3 && u[0] == 102 && u[1] == 102 && u[2] == 108) {
            msg("<warning> <scireum> Reducing a multi-byte ligature to ffl", 0);
            g->unicode = 0xFB04;
        } else {
            if (uLen > 1) {
                msg("<warning> <scireum> File contains multi-byte unicode mappings which are unsupported!", 0);
            }
            g->unicode = u[0];
        }
    }
...
GBool InfoOutputDev::beginType3Char(GfxState *state, double x, double y, double dx, double dy, CharCode code,
                                    Unicode *u, int uLen) {
    GfxFont *font = state->getFont();
    if (!font)
        return gTrue;
...
void InfoOutputDev::drawMaskedImage(GfxState *state, Object *ref, Stream *str,
    ...
    OutputDev::drawMaskedImage(state, ref, str, width, height, colorMap, POPPLER_INTERPOLATE_ARG maskStr, maskWidth,
                               maskHeight, maskInvert POPPLER_MASK_INTERPOLATE_ARG);
}
...
void InfoOutputDev::drawSoftMaskedImage(GfxState *state, Object *ref, Stream *str,
    ...
    OutputDev::drawSoftMaskedImage(state, ref, str, width, height, colorMap, POPPLER_INTERPOLATE_ARG maskStr, maskWidth,
                                   maskHeight, maskColorMap POPPLER_MASK_INTERPOLATE_ARG);
}
